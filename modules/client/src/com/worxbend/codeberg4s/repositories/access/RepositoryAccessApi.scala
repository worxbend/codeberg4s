...
import com.worxbend.codeberg4s.repositories.access.wire.{AccessQueries, AddCollaboratorOptionDto, CreateKeyOptionDto}
...
/** A repository's access control surface: which accounts and teams are collaborators, and which machines hold a key.
  *
  * The rules that stop a push once someone is in — branch and tag protections — are on [[protections]], a group of its
  * own: this class answers who may reach the repository, and that one answers what they may do to a ref.
  *
...
final class RepositoryAccessApi private[codeberg4s] (pipeline: ApiPipeline[Future])(using exec: Exec[Future]):
  ...
  /** The rules that stop a push: branch protections and tag protections. */
  val protections: RepositoryProtectionApi = RepositoryProtectionApi(pipeline)
