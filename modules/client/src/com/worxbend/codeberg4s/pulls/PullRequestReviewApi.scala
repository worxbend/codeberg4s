...
import com.worxbend.codeberg4s.pulls.PullRequestRequests.{pullPath, reviewPath, reviewsPath}
...
/** The review half of the pull-request surface: the verdicts on a pull request, and who has been asked to write one.
  *
  * The remarks anchored to diff lines inside a review are on [[comments]], a group of its own: they are addressed one
  * level deeper, by review '''and''' comment.
  *
...
final class PullRequestReviewApi private[codeberg4s] (pipeline: ApiPipeline[Future])(using exec: Exec[Future]):
  ...
  val comments: ReviewCommentApi = ReviewCommentApi(pipeline)
