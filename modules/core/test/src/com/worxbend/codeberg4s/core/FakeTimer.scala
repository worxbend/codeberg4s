...
/** A [[Timer]] that records what it was asked to wait for instead of waiting.
  ...
  * `Retry-After` — is observable without a single real millisecond passing. The simulated clock advances by exactly the
  * requested duration, which keeps [[nowMillis]] consistent with the sleeps a test asked for.
  *
  * @param startMillis
