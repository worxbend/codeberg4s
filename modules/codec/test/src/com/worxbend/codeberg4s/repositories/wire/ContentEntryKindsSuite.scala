...
final class ContentEntryKindsSuite extends FunSuite with GoldenFixtures:
  ...
  test("a symlink entry becomes Symlink and carries what the link points at"):
    entry(DirectoryCapture, "type" -> JsonValue.Str("symlink"), "target" -> JsonValue.Str("../shared/README.md")) match
  ...
  test("a symlink whose target the instance did not send is still a symlink, with no target"):
    entry(DirectoryCapture, "type" -> JsonValue.Str("symlink"), "target" -> JsonValue.Null) match
  ...
  test("a submodule entry becomes Submodule and carries the clone url .gitmodules records"):
    ...
    entry(
      DirectoryCapture,
      "type"              -> JsonValue.Str("submodule"),
      "submodule_git_url" -> JsonValue.Str(gitmodules),
    ) match
  ...
  test("a submodule the repository records no clone url for is still a submodule"):
    entry(DirectoryCapture, "type" -> JsonValue.Str("submodule"), "submodule_git_url" -> JsonValue.Null) match
  ...
  test("the type alone decides the case, so a payload's file bytes do not follow it onto another kind"):
    entry(FileCapture, "type" -> JsonValue.Str("symlink"), "target" -> JsonValue.Str("docs/README.md")) match
      case ContentEntry.Symlink(meta, target) =>
  ...
  test("an encoding this library does not implement keeps the payload verbatim and refuses to guess at bytes"):
    fileContent(entry(FileCapture, "encoding" -> JsonValue.Str("hex"))) match
  ...
  test("content sent without any encoding is kept verbatim rather than assumed to be base64"):
    fileContent(entry(FileCapture, "encoding" -> JsonValue.Null)) match
      case Some(payload @ FileContent.Opaque(encoding, raw)) =>
  ...
  test("the encoding name is matched ignoring case and surrounding space, since only its spelling varies"):
    fileContent(entry(FileCapture, "encoding" -> JsonValue.Str("  BASE64 "))) match
  ...
  test("an encoding without content is nothing at all, because neither field means anything alone"):
    fileContent(entry(FileCapture, "content" -> JsonValue.Null)) match
      case None  => ()
  ...
  private def derived(fixture: String, overrides: Seq[(String, JsonValue)]): Map[String, JsonValue] =
    val parsed = Json.parse(golden(fixture)).getOrElse(fail(s"$fixture is not valid JSON"))
  ...
  private def entry(fixture: String, overrides: (String, JsonValue)*): ContentEntry =
    val body = Json.render(JsonValue.Obj.from(derived(fixture, overrides)))
