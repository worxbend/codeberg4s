...
final class TimelineCommentDtoSuite extends FunSuite:
  ...
  test("every declared field decodes when present"):
    ...
    assertEquals(event.author.map(_.login), Some("actor"))
    assertEquals(event.assignee.map(_.login), Some("assigned"))
    ...
    assertEquals(event.resolvedBy.map(_.login), Some("resolver"))
