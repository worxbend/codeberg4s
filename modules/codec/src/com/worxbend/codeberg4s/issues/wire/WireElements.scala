...
/** Converts the elements of a decoded JSON array, reporting the position of whichever one failed.
  ...
  * Writing the fold once means a decoding failure says `$[2].id` or `$.labels[1].name` rather than `$`, and means the
  * five call sites cannot drift into disagreeing about whether one bad element fails the page.
  *
...
private[codeberg4s] object WireElements:
  ...
  def at[D, A](base: JsonPath, dtos: Vector[D])(
    ...
    dtos.zipWithIndex.foldLeft[Either[DecodeFailure, Vector[A]]](Right(Vector.empty)):
      case (soFar, (dto, position)) =>
        soFar.flatMap(converted => convert(dto, base.index(position)).map(element => converted.appended(element)))
