...
/** Converting a JSON array of DTOs into domain values, with each failure reported at its own index.
  ...
  * could not convert. For an array that means `$[7].sha` and not `$`, and getting there requires threading the index
  * through the fold. Doing it once here is what stops every list-shaped model from growing its own copy of the same
  * four lines — and a copy that quietly forgot the index would be indistinguishable from one that did not, until
  * someone tried to debug a bad payload.
  *
...
private[codeberg4s] object Elements:
  ...
  def convert[D, A](at: JsonPath, dtos: Vector[D])(
    ...
    dtos.zipWithIndex.foldLeft[Either[DecodeFailure, Vector[A]]](Right(Vector.empty)):
      case (Left(failure), _)           => Left(failure)
      case (Right(converted), (dto, i)) => one(dto, at.index(i)).map(converted.appended)
