...
/** The body of `GET /repos/{owner}/{repo}/editorconfig/{filepath}` — a bare object of property names to values.
  ...
  *
  * A whole number renders without a fractional part: `ujson` parses every JSON number as a `Double`, so `4` arrives as
  * `4.0` and would read as `"4.0"` if it were rendered naively. That is the one piece of arithmetic in this file and
...
object EditorConfigDto:
  ...
  given upickle.default.Reader[EditorConfigDto] =
  ...
  private def rendered(value: ujson.Value): Option[String] =
    value.strOpt
