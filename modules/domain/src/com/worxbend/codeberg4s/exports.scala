 ...
 *
 * A caller writing the quick-start flow — authenticate with an `ApiToken`, build an `Auth`,
 * construct a client, page through a listing — should not need to know which sub-package each of
 * those types lives in. These exports make `import com.worxbend.codeberg4s.*` sufficient for that flow.
 *
...
export com.worxbend.codeberg4s.auth.ApiToken
export com.worxbend.codeberg4s.auth.Auth
export com.worxbend.codeberg4s.paging.Page
export com.worxbend.codeberg4s.paging.PageNumber
