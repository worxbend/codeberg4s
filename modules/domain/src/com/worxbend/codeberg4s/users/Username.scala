...
/** The handle that names a person — the `{username}` of `/users/{username}`.
  *
  * This is deliberately '''not''' [[com.worxbend.codeberg4s.repositories.Owner]], even though the two are the same
  * characters on the wire. An `Owner` answers "who does this repository belong to?", and the answer may be an
  * organisation; a `Username` answers "which account is this?", and the endpoints that take one —
  * `/users/{u}/followers`, `/users/{u}/keys` — are about a person. Giving them one type would let
  * `client.users.keys(repository.slug.owner)` compile against an organisation that has no keys, which is exactly the
  * confusion the type is here to prevent. Converting is a deliberate step through [[from]], not an implicit widening.
  *
