...
/** A closed set whose members each know the exact word Forgejo puts on the wire for them.
  ...
  * back into a request. Before this trait existed each of them spelled that out twice — once as a `parse` matching wire
  * words to cases, once as a renderer matching cases back to wire words — and the two lists could drift apart without
  * the compiler noticing. Worse, the renderer was not even called the same thing everywhere: most enums named it
  * `wireName` and one named it `wireValue`.
  *
...
object WireVocabulary:
  ...
  /** Finds the member of `values` whose [[WireVocabulary.wireName]] matches `value`, or `None` if there is no match.
    *
    * The incoming string is trimmed and lowercased before the comparison, so a field that arrives as `" Public "` still
    * resolves. Lowercasing uses `Locale.ROOT` rather than the JVM's default locale: under a Turkish default locale
    * `"PRIVATE".toLowerCase` yields `"prıvate"` with a dotless `ı`, which would match nothing and turn a perfectly good
    * response into a silent `None` on one machine and not another.
    *
