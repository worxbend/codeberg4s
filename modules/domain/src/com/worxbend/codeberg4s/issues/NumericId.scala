...
/** Validation shared by every identifier in this group that Forgejo expresses as a positive integer.
  ...
  * interpolated into a request path. Unlike [[com.worxbend.codeberg4s.Owner]] a number cannot forge a path, so the
  * point here is not escaping but confusion: an issue's per-repository `number` and its instance-wide `id` are both
  * `Long` and are routinely mixed up, and `/issues/0` is a request Forgejo answers with a `404` that reads like a
  * missing issue rather than like a caller bug. Rejecting non-positive values once, here, keeps both problems out of
  * the four opaque types.
  */
